...
> [!WARNING]
> Vá à seção avisos e veja as notas deixadas lá sobre o `batch processing`.
...
## Melhorias futuras
- Adicionar um GUI, facilitando navegação e apresentação do programa.
- Melhorar as importações em lote.
- Adicionar mais opções de ônibus.
...
This option lists all available lines; then the user provides the destination city and desired schedule, and the system shows available seats for that configuration (if any).  
After listing seats, the user is asked if they want to purchase; if yes, required information is requested and the purchase is completed; otherwise it returns to the tickets menu.

![available system](.assets/available_seats.png)

### 2.5.3 Batch import (from file)

There is a function that loads multiple tickets from a file — each line of the file represents a ticket. Since the file contains only one city, the strategy was: check if this city matches the origin or destination of any registered line and, together with date and time, if there is a match, add the ticket to that line.
...
> [!WARNING]
> Go to the warnings section and see the notes left there regarding `batch processing`.
...
The RouteManager program fulfills, in a simple way, the goal of creating and managing bus lines and tickets, offering functions ranging from line creation and editing to ticket sales, batch processing, and revenue and occupancy reports.


# 2.8 Limitations and Future Improvements

## Limitations
- The program works only through the command line (no graphical interface).
- The system assumes buses have a fixed capacity of 20 seats.
- Batch imports may generate errors when the file data does not exactly match the registered lines.

## Future Improvements
- Add a GUI to facilitate navigation and program presentation.
- Improve batch imports.
...
> [!WARNING]  
> Due to the input format for batch processing, many ticket imports may generate errors, as in this mode the file presents only one city, while the program uses two cities (departure and destination). Thus, without the second variable (city), the program does not know what to do. **The program currently checks if the city is among the destinations or origins already registered in the system; if positive, it checks date and time, and if these match, then it adds the ticket.**
